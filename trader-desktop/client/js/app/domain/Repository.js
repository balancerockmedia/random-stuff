    ...
    function(Backbone, Socket, Orders) {
        ...
        var _repository = {
            ...
            createRandomOrder: function() {
                // There might be a better way to do this, passing _instruments as a
                // param doesn't seem like the best idea...
                _orders.createRandomOrder(_instruments, _loggedInUser);
            }
        };
        ...
        Socket.on('placementCreatedEvent', function(placement) {
            var model = _orders.get(placement.orderId);
            model.set(placement);
        });

        Socket.on('executionCreatedEvent', function(execution) {
            var model = _orders.get(execution.orderId);
            model.set(execution);
        });
